    ...
    if fov_recompute {
        ...
        for y in 0..MAP_HEIGHT {
            for x in 0..MAP_WIDTH {
                ...
                if *explored {
                    // show explored tiles only (any visible tile is explored alredy)
...
fn main() {
    ...
    while !root.window_closed() {
        ...
        // let monsters take their TookTurn
        if objects[PLAYER].alive && player_action != PlayerAction::DidntTakeTurn {
            for object in &objects {
                // only if object is not player_action
